...
func (env *Environment) BroadcastTxAsync(_ *rpctypes.Context, app string, tx types.Tx) (*ctypes.ResultBroadcastTx, error) {
	err := env.Mempool.CheckTx(app, tx, nil, mempl.TxInfo{})
	if err != nil {
		return nil, err
	}
...
func (env *Environment) BroadcastTxSync(ctx *rpctypes.Context, app string, tx types.Tx) (*ctypes.ResultBroadcastTx, error) {
	resCh := make(chan *abci.ResponseCheckTx, 1)
	err := env.Mempool.CheckTx(app, tx, func(res *abci.ResponseCheckTx) {
		select {
...
func (env *Environment) BroadcastTxCommit(ctx *rpctypes.Context, app string, tx types.Tx) (*ctypes.ResultBroadcastTxCommit, error) {
...
func (env *Environment) CheckTx(_ *rpctypes.Context, app string, tx types.Tx) (*ctypes.ResultCheckTx, error) {
	res, err := env.ProxyAppMempool.CheckTx(context.TODO(), &abci.RequestCheckTx{Tx: tx, App: app})
	if err != nil {
		return nil, err
	}
